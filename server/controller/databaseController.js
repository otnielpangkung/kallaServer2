...
class DatabaseController {
	...
	static addRealisasi(req, res) {
		...
		NomorWo.findOne({
			...
			.then((data) => {
				...
				return Realisasi.create(payload);
			})
			.then((data) => {
				res.status(200).json(data);
			})
	...
	static editRealisasi(req, res) {
		NomorWo.findOne({
			...
			.then((data) => {
				...
				return Realisasi.update(payload, {
					where: {
						id: req.params.id,
					},
				});
	...
	static getRelasiPeriod(req, res) {
		let formatStartDate = moment(req.query.startDate)
			.startOf('day')
			.format('YYYY-MM-DD HH:mm:ss');
		let formatEndDate = moment(req.query.endDate)
			.endOf('day')
			.format('YYYY-MM-DD HH:mm:ss');
	...
	static getAllRealisasi(req, res) {
		Realisasi.findAndCountAll({
			limit: req.query.limit,
			offset: req.query.offset,
			include: [Schedule, Relasi, User, Item, NomorWo],
			order: [
				['tanggal', 'desc'],
				['UserId', 'asc'],
			],
	...
	static getAllRelasi(req, res) {
		Relasi.findAndCountAll({
			limit: req.query.limit,
			offset: req.query.offset,
			include: [Schedule, Realisasi, User],
		})
			.then((data) => {
				// console.log(data, "=======");
				res.status(200).json(data);
